...
from flaskblog import bcrypt, db, params
...
def register():
    ...
    return render_template("register.html", title="Register", form=form, params=params)
...
def account():
    ...
    return render_template("account.html", title="Account", params=params, form=form)
...
def user_posts(username):
    ...
    if category is None:
        ...
        posts = (
            Post.query.filter_by(author=user, category=category)
            .order_by(Post.date_posted.desc())
            .paginate(page=page, per_page=params["posts_per_page"])
        )
    # print(paragraphs)
    return render_template(
        ...
        params=params,
        user=user,
        user_post_categories=categories,
        user_post_category=category,
    )
...
def reset_request():
    ...
    return render_template(
        "reset_request.html", title="Reset Password", form=form, params=params
    )
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            f"Your password has been updated! You are now able to log in",
            "success",
        )
        return redirect(url_for("users.login"))
    return render_template(
        "reset_token.html", title="Reset Password", form=form, params=params
    )
